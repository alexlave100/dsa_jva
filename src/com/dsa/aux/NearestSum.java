...
public static int[] closestSumPair(int[] a1, int[] a2, int target) {
  int[] a1Sorted = Arrays.copyOf(a1, a1.length); // 1 - 4 - 7 - 10
  Arrays.sort(a1Sorted);
  int[] a2Sorted = Arrays.copyOf(a2, a2.length); // 4 - 5 - 7 - 8
  ...
  while (i < a1Sorted.length && j >= 0 ) {
      int v1 = a1Sorted[i]; // 1, 4, 7, 10
      int v2 = a2Sorted[j]; // 8, 7, 5
      int currentDiff = v1 + v2 - target; // -4, -1, 2, 1, -1, 1
      if (Math.abs(currentDiff) < smallestDiff) {
          smallestDiff = Math.abs(currentDiff); // 4, 1
          closestPair[0] = v1; closestPair[1] = v2; // {1, 8}, {4, 8}
      }
